...
class Tossing_PID():
    def __init__(self, train_x, train_y, test_x, test_y, G, D, Q, device, nepochs, lambda_val, noise_dim):
        super(Tossing_PID, self).__init__()
...
#         self.x_f = torch.cat([self.train_x, self.test_x], dim = 0)
    ...
    def expo_transformation(self, phyloss):
        probs = torch.unsqueeze(torch.exp(- self.lambda_val * torch.abs(phyloss)),1)
        return probs
    ...
    def train_discriminator(self, x, y):
        ...
        ## REAL DATA         
        real_prob = self.expo_transformation(self.physics_loss(x, y, [self.Xmean, self.Xstd], [self.Ymean, self.Ystd]))
        ...
        phy_prob = self.expo_transformation(self.physics_loss(x, y_pred, [self.Xmean, self.Xstd], [self.Ymean, self.Ystd]))
        fake_logits = self.D.forward(torch.cat([x, y_pred, phy_prob], dim=1))

        ## UNLABELLED DATA SAMPLES
        D_noise_unlabelled = self.sample_noise(self.test_x.shape[0], self.noise_dim).to(self.device)
        y_pred_unlabelled = self.G.forward(torch.cat([self.test_x, D_noise_unlabelled], dim=1))
        phy_prob_unlabelled = self.expo_transformation(self.physics_loss(self.test_x, y_pred_unlabelled, [self.Xmean, self.Xstd], [self.Ymean, self.Ystd]))
        fake_logits_unlabelled = self.D.forward(torch.cat([self.test_x, y_pred_unlabelled, phy_prob_unlabelled], dim=1))  

        d_loss = self.discriminator_loss(real_logits, fake_logits, fake_logits_unlabelled)
    ...
    def train_generator(self, x, y):
        ...
        ## UNLABELLED DATA SAMPLES
        G_noise_unlabelled = self.sample_noise(self.test_x.shape[0], self.noise_dim).to(self.device)
        y_pred_unlabelled = self.G.forward(torch.cat([self.test_x, G_noise_unlabelled], dim=1))
        phy_prob_unlabelled = self.expo_transformation(self.physics_loss(self.test_x, y_pred_unlabelled, [self.Xmean, self.Xstd], [self.Ymean, self.Ystd]))

        fake_logits_unlabelled = self.D.forward(torch.cat([self.test_x, y_pred_unlabelled, phy_prob_unlabelled], dim=1)) 
            
        mse_loss = torch.nn.functional.mse_loss(y_pred, y)
        phy_loss = torch.mean(torch.abs(self.physics_loss(x, y_pred, [self.Xmean, self.Xstd], [self.Ymean, self.Ystd])))  
            
        z_pred = self.Q.forward(torch.cat([x, y_pred], dim=1))
        mse_loss_Z = torch.nn.functional.mse_loss(z_pred, G_noise)

        adv_loss = self.generator_loss(fake_logits, fake_logits_unlabelled) 
